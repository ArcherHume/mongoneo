...
from mongoneo.base.utils import NonOrderedList
...
class EmbeddedDocument(BaseDocument, metaclass=DocumentMetaclass):
    r"""A :class:`~mongoneo.Document` that isn't stored in its own
    collection.  :class:`~mongoneo.EmbeddedDocument`\ s should be used as
    fields on :class:`~mongoneo.Document`\ s through the
    :class:`~mongoneo.EmbeddedDocumentField` field type.

    A :class:`~mongoneo.EmbeddedDocument` subclass may be itself subclassed,
    to create a specialised version of the embedded document that will be
    stored in the same collection. To facilitate this behaviour a `_cls`
    field is added to documents (hidden though the MongoNeo interface).
    To enable this behaviour set :attr:`allow_inheritance` to ``True`` in the
    :attr:`meta` dictionary.
    """

    # Add a flag to indicate if the document is an embedded document
    _is_embedded = True
...
class Document(BaseDocument, metaclass=TopLevelDocumentMetaclass):
    ...
    :class:`~mongoneo.Document` subclass.

    By default, the MongoDB collection used to store documents created using a
    :class:`~mongoneo.Document` subclass will be the name of the subclass
    converted to snake_case. A different collection may be specified by
    providing :attr:`collection` to the :attr:`meta` dictionary in the class
    definition.

    A :class:`~mongoneo.Document` subclass may be itself subclassed, to
    ...
    A :class:`~mongoneo.Document` may use a **Capped Collection** by
    specifying :attr:`max_documents` and :attr:`max_size` in the :attr:`meta`
    dictionary. :attr:`max_documents` is the maximum number of documents that
    is allowed to be stored in the collection, and :attr:`max_size` is the
    maximum size of the collection in bytes. :attr:`max_size` is rounded up
    to the next multiple of 256 by MongoDB internally and mongoneo before.
    ...
    False then indexes will not be created by MongoNeo.  This is useful in
    ...
    in your model will raise a :class:`~mongoneo.FieldDoesNotExist` error.
    ...
    def save(
        ...
        **kwargs,
        ...
        """Save the :class:`~mongoneo.Document` to the database. If the
        ...
        .. versionchanged:: 0.26
           save() no longer calls :meth:`~mongoneo.Document.ensure_indexes`
        ...
        # and turned into mongoneo.OperationError
        ...
        if update_doc:
            ...
            if last_error is not None:
                ...
                if updated_existing is False:
                    ...
                    # https://github.com/MongoNeo/mongoneo/issues/564
    ...
    def _object_key(self):
        ...
        Note that the dict returned by this method uses MongoNeo field
    ...
    def update(self, **kwargs):
        """Performs an update on the :class:`~mongoneo.Document`
        A convenience wrapper to :meth:`~mongoneo.QuerySet.update`.
    ...
    def delete(self, signal_kwargs=None, **write_concern):
        """Delete the :class:`~mongoneo.Document` from the database. This
    ...
    def switch_db(self, db_alias, keep_created=True):
        ...
        .. seealso::
            Use :class:`~mongoneo.context_managers.switch_collection`
    ...
    def switch_collection(self, collection_name, keep_created=True):
        ...
        .. seealso::
            Use :class:`~mongoneo.context_managers.switch_db`
    ...
    def _reload(self, key, value):
        """Used by :meth:`~mongoneo.Document.reload` to ensure the
    ...
    def drop_collection(cls):
        ...
        :class:`~mongoneo.Document` type from the database.
    ...
    def compare_indexes(cls):
        """Compares the indexes defined in MongoNeo with the ones
...
class DynamicDocument(Document, metaclass=TopLevelDocumentMetaclass):
    ...
    schemas.  As a :class:`~mongoneo.Document` subclass, acts in the same
    way as an ordinary document but has expanded style properties.  Any data
    passed or set against the :class:`~mongoneo.DynamicDocument` that is
    not a field is automatically converted into a
    :class:`~mongoneo.fields.DynamicField` and data can be attributed to that
    field.
...
class DynamicEmbeddedDocument(EmbeddedDocument, metaclass=DocumentMetaclass):
    ...
    uncontrolled schemas. See :class:`~mongoneo.DynamicDocument` for more
...
class Index:
    ...
    def to_index_spec(self):
        """Convert to index specification format used by MongoNeo."""
        if self._compound:
            fields = []
    ...
    def decorator(cls):
        ...
        if kwargs.get("soft_delete", False):
            if not hasattr(cls, "is_deleted"):
                from mongoneo.fields import BooleanField
        ...
        from mongoneo.query_builder import enhance_model_class

        enhance_model_class(new_cls)
