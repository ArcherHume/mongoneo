...
from mongoneo.queryset.field_list import QueryFieldList
from mongoneo.queryset.visitor import Q, QNode
...
class BaseQuerySet:
    ...
    providing :class:`~mongoneo.Document` objects as the results.
    """
    ...
    def __call__(self, q_obj=None, **query):
        ...
        :class:`~mongoneo.queryset.QuerySet` with a query.

        :param q_obj: a :class:`~mongoneo.queryset.Q` object to be used in
            the query; the :class:`~mongoneo.queryset.QuerySet` is filtered
            multiple times with different :class:`~mongoneo.queryset.Q`
    ...
    def __getstate__(self):
        ...
        See https://github.com/MongoNeo/mongoneo/issues/442
        """
    ...
    def filter(self, *q_objs, **query):
        """An alias of :meth:`~mongoneo.queryset.QuerySet.__call__`"""
    ...
    def get(self, *q_objs, **query):
        ...
        :class:`~mongoneo.queryset.MultipleObjectsReturned` or
        `DocumentName.MultipleObjectsReturned` exception if multiple results
        and :class:`~mongoneo.queryset.DoesNotExist` or
    ...
    def _clone_into(self, new_qs):
        ...
        :class:`~mongoneo.queryset.base.BaseQuerySet`).
        """
    ...
    def only(self, *fields):
        ...
        :func:`~mongoneo.queryset.QuerySet.all_fields` will reset any
        field filters.
    ...
    def order_by(self, *keys, __raw__=None):
        """Order the :class:`~mongoneo.queryset.QuerySet` by the given keys.
    ...
    def explain(self):
        ...
        :class:`~mongoneo.queryset.QuerySet` cursor.
        """
    ...
    def scalar(self, *fields):
        ...
        :func:`~mongoneo.queryset.QuerySet.no_dereference` to turn off
        dereferencing.
    ...
    def map_reduce(
        ...
        See the :meth:`~mongoneo.tests.QuerySetTest.test_map_reduce`
        and :meth:`~mongoneo.tests.QuerySetTest.test_map_advanced`
        ...
        :class:`~mongoneo.document.MapReduceDocument`.
        """
        queryset = self.clone()
    ...
    def exec_js(self, code, *fields, **options):
        ...
        As fields in MongoNeo may use different names in the database (set
        using the :attr:`db_field` keyword argument to a :class:`Field`
        constructor), a mechanism exists for replacing MongoNeo field names
        with the database field names in Javascript code. When accessing a
        field, use square-bracket notation, and prefix the MongoNeo field
        name with a tilde (~).
    ...
    def where(self, where_clause):
        ...
        :meth:`mongoneo.queryset.Queryset.exec_js`.
    ...
    def item_frequencies(self, field, normalize=False, map_reduce=True):
        ...
        .. note::
            ...
            :class:`~mongoneo.fields.ReferenceField` or
            :class:`~mongoneo.fields.GenericReferenceField` for more complex
            counting a manual map reduce call is required.

        If the field is a :class:`~mongoneo.fields.ListField`, the items within
    ...
    def __next__(self):
        """Wrap the result in a :class:`~mongoneo.Document` object."""
        if self._none or self._empty:
            raise StopIteration
    ...
    def _get_order_by(self, keys):
        """Given a list of MongoNeo-style sort keys, return a list
    ...
    def aggregate(self, pipeline=None):
        ...
        This method allows running MongoDB aggregation pipelines on the collection
        and returns the results as Document instances.

        Args:
            pipeline: A list of aggregation pipeline stages.

        Returns:
            A list of transformed Document instances.
        """
        ...
        # Process and convert raw results to Document instances
        documents = []
        for doc in results:
            # Filter out lookup fields (those that start with "__")
            filtered_doc = {k: v for k, v in doc.items() if not k.startswith("__")}
            documents.append(self._document._from_son(filtered_doc))
