...
from mongoneo.connection import (
...
from mongoneo.queryset import DO_NOTHING
from mongoneo.queryset.base import BaseQuerySet
from mongoneo.queryset.transform import STRING_OPERATORS
...
class StringField(BaseField):
    ...
    def __init__(self, regex=None, max_length=None, min_length=None, **kwargs):
        ...
        :param kwargs: Keyword arguments passed into the parent :class:`~mongoneo.BaseField`
        """
...
class URLField(StringField):
    ...
    def __init__(self, url_regex=None, schemes=None, **kwargs):
        ...
        :param kwargs: Keyword arguments passed into the parent :class:`~mongoneo.StringField`
        """
...
class DecimalField(BaseField):
    ...
    is not suitable for true decimal storage. Consider using :class:`~mongoneo.fields.Decimal128Field`.
...
class DateTimeField(BaseField):
    ...
    Note: Microseconds are rounded to the nearest millisecond.
      ...
      Use :class:`~mongoneo.fields.ComplexDateTimeField` if you
...
class EmbeddedDocumentField(BaseField):
    ...
    Only valid values are subclasses of :class:`~mongoneo.EmbeddedDocument`.
    """
    ...
    def __init__(self, document_type, **kwargs):
        ...
        # Clone all fields from the document_type
        self._fields = document_type._fields.copy()
        super().__init__(**kwargs)
    ...
    def __getattribute__(self, name: str):
        try:
            ...
            if name in fields:
                # Get the field
                field = fields[name]
                # Create a shallow copy of the field to avoid modifying the original
                cloned_field = copy.copy(field)
                # Set the full field name with parent path
                current_name = object.__getattribute__(self, "name")
                if current_name is not None:  # Only set prefix if this field has a name
                    cloned_field.name = f"{current_name}.{name}"
                else:
                    cloned_field.name = name
                return cloned_field
        except (AttributeError, KeyError):
            pass
        # Fallback to normal attribute lookup
        return super().__getattribute__(name)

    def __set_owner_document__(self, owner_document):
...
class GenericEmbeddedDocumentField(BaseField):
    ...
    :class:`~mongoneo.EmbeddedDocument` to be stored.
...
class DynamicField(BaseField):
    ...
    Used by :class:`~mongoneo.DynamicDocument` to handle dynamic data"""
...
class EmbeddedDocumentListField(ListField):
    """A :class:`~mongoneo.ListField` designed specially to hold a list of
    embedded documents to provide additional query helpers.

    .. note::
        The only valid list values are subclasses of
        :class:`~mongoneo.EmbeddedDocument`.
    """

    def __init__(self, document_type, **kwargs):
        """
        :param document_type: The type of
         :class:`~mongoneo.EmbeddedDocument` the list will hold.
        :param kwargs: Keyword arguments passed into the parent :class:`~mongoneo.ListField`
        """
...
class ReferenceField(BaseField):
    ...
    this field. This is necessary because the field returns a :class:`~mongoneo.Document`
    ...
    :class:`~mongoneo.fields.LazyReferenceField`.
    ...
    The options are:
      ...
      * PULL       (4)  - Pull the reference from a :class:`~mongoneo.fields.ListField` of references
    ...
    def __getattribute__(self, name: str):
        try:
            # First check if it's a valid attribute/method on the class
            return super().__getattribute__(name)
        except AttributeError:
            # If not a normal attribute, try to access a field from the document type
            doc_type = super().__getattribute__("document_type")
            if hasattr(doc_type, "_fields") and name in doc_type._fields:
                # Get the field from the document type
                field = doc_type._fields[name]
                # Create a shallow copy of the field
                import copy

                cloned_field = copy.copy(field)
                # For the QueryBuilder approach, we need to keep the path intact
            ...
            raise AttributeError(f"'ReferenceField' object has no attribute '{name}'")
...
class GenericReferenceField(BaseField):
    """A reference to *any* :class:`~mongoneo.document.Document` subclass
    that will be automatically dereferenced on access (lazily).

    Note this field works the same way as :class:`~mongoneo.document.ReferenceField`,
    ...
    :class:`~mongoneo.fields.GenericLazyReferenceField`.
...
class SequenceField(BaseField):
    ...
    :param collection_name:  Name of the counter collection (default 'mongoneo.counters')
    ...
    _auto_gen = True
    COLLECTION_NAME = "mongoneo.counters"
    VALUE_DECORATOR = int
...
class LazyReferenceField(BaseField):
    ...
    Unlike the :class:`~mongoneo.fields.ReferenceField` it will
    **not** be automatically (lazily) dereferenced on access.
    Instead, access will return a :class:`~mongoneo.base.LazyReference` class
    ...
    def __init__(
        ...
        **kwargs,
        ...
        :param passthrough: When trying to access unknown fields, the
          :class:`~mongoneo.base.datastructure.LazyReference` instance will
...
class GenericLazyReferenceField(GenericReferenceField):
    """A reference to *any* :class:`~mongoneo.document.Document` subclass.
    Unlike the :class:`~mongoneo.fields.GenericReferenceField` it will
    **not** be automatically (lazily) dereferenced on access.
    Instead, access will return a :class:`~mongoneo.base.LazyReference` class
    instance, allowing access to `pk` or manual dereference by using
    ``fetch()`` method.
